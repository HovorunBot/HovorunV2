...
class NotificationService:
    ...
    async def notify_updates(self, bot: Bot) -> None:
        """Check for version updates and notify whitelisted chats."""
        ...
        if not whitelisted_chats:
            logger.info("No whitelisted chats to notify.")
            await self._system_service.set_last_notified_version(current_version)
            return

        logger.info("Broadcasting update notification to %d chats...", len(whitelisted_chats))
